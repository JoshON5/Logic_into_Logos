	...
	async run() {
		...
		const questions = [
			...
			},
			{
				...
				name: "color",
				message:
					"Enter in the color value for the shape you would like (for hex colors start with #):",
			},
		];
		...
		if (!hexColors.test(data.color) && !colors === colorsCheck) {
			throw new Error(
				"please enter in a valid hex color or color input.(refer to colorChoices.js file for valid colors)"
			);
		}
